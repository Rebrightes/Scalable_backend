...
const blobServiceClient = BlobServiceClient.fromConnectionString(blobStorage.connectionString);
const containerClient = blobServiceClient.getContainerClient(blobStorage.containerName);
...
export async function uploadVideo(req, res) {
  ...
  if (!file.originalname.match(/\.(mp4|avi|mov)$/i)) {
    return res.status(400).send("Invalid file type. Only MP4, AVI, or MOV are allowed.");
  }
