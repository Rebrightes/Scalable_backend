...
export async function getUploadedVideos(req, res) {
  ...
  const videosWithAverageRating = videos.map(video => {
    const averageRating = video.ratings?.length ? video.ratings.reduce((sum, rating) => sum + rating.value, 0) / video.ratings.length : 0;
    return {
      ...video,
      averageRating
    };
...
export async function getVideoDetails(req, res) {
  ...
  try {
    const {
      resource: video
    } = await container.item(videoId).read();
    if (!video) {
...
export async function rateVideo(req, res) {
  ...
  // Calculate the average rating
  const averageRating = video.ratings.reduce((sum, rating) => sum + rating.value, 0) / video.ratings.length;
  res.json({
    ...video,
    averageRating
  });
